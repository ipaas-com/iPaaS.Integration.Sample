...
namespace FakeStore.Data.Interface
{
    public class TranslationUtilities : Integration.Abstract.TranslationUtilities
    {
        ...
        public override string GetPrimaryId(Integration.Abstract.Connection connection, int mappingCollectionType, object SourceObject, long? MappingCollectionId = null)
        {
            ...
            //These two are special cases.  They are not AbstractIntegrationData, but they do have a primary id that is copied from the parent.
            if (mappingCollectionType == (int)TM_MappingCollectionType.CUSTOMER_ADDRESS)
                return Convert.ToString(((UserAddress)SourceObject).UserId);
            if (mappingCollectionType == (int)TM_MappingCollectionType.TRANSACTION_LINE)
                return ((CartProduct)SourceObject).GetPrimaryId();
        ...
        public override List<ChildMapping> GetChildMappings(Integration.Abstract.Connection connection, int mappingCollectionType, long mappingResponseId, int mappingDirection)
        {
            ...
            {
                case TM_MappingCollectionType.TRANSACTION:
                    retVal.Add(new ChildMapping() { Field = "Products", MappingCollectionType = (int)TM_MappingCollectionType.TRANSACTION_LINE });
                    break;
                case TM_MappingCollectionType.CUSTOMER:
                    retVal.Add(new ChildMapping() { Field = "Address", MappingCollectionType = (int)TM_MappingCollectionType.CUSTOMER_ADDRESS });
                    break;
                default:
                    break;
            }
